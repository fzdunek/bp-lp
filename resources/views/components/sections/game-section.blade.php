    ...
    <div class="relative z-10 w-full h-full flex flex-col items-center justify-center px-4 sm:px-8 py-12 md:py-16">
        <div class="relative w-full max-w-6xl mx-auto space-y-8 md:space-y-12">
            <div class="text-center space-y-4 md:space-y-6">
                <h2 class="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold text-cream leading-tight">
                    {{ $title }}
                </h2>
            ...
            <div class="flex justify-center mt-8 md:mt-12">
                <div class="w-full max-w-xs md:max-w-sm h-[600px] md:h-[800px] rounded-[20px] border-2" style="background-color: #F5F5DC; border-color: #1a1a1a;">
                    <div class="w-full h-full flex items-center justify-center">
                        @if($gameIframeUrl)
                            ...
                            ></iframe>
                        @else
                            <p class="text-gray-500 text-lg font-medium">{{ $gamePlaceholder }}</p>
                        @endif
                    </div>
                </div>
            </div>
